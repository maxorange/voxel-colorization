...
def write(voxel_model, fp):
    ...
    fp.write(bytes('#binvox 1\n', 'latin-1'))
    fp.write(bytes('dim '+' '.join(map(str, voxel_model.dims))+'\n', 'latin-1'))
    fp.write(bytes('translate '+' '.join(map(str, voxel_model.translate))+'\n', 'latin-1'))
    fp.write(bytes('scale '+str(voxel_model.scale)+'\n', 'latin-1'))
    fp.write(bytes('data\n', 'latin-1'))
    ...
    for c in voxels_flat:
        if c==state:
            ...
            if ctr==255:
                fp.write(bytes(chr(state), 'latin-1'))
                fp.write(bytes(chr(ctr), 'latin-1'))
                ctr = 0
        else:
